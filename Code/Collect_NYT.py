...
def get_content(feed_name, feed_link):
  ...
  get_latest = pd.DataFrame(columns = ['Feed','Publish_DT','Title','Summary','Author','Link'])
...
print("==== Starting data fetch for NYT ====")
...
for index, row in table_sources.iterrows():
  ...
  print("Fetching data for NYT: " + feed_name)
...
table_content = pd.read_csv("Output/NYT.csv")
# Backup data file
...
table_content.drop_duplicates(subset=['Title', 'Summary'], keep='last', inplace=True)
# Sort all data
table_content.sort_values("Publish_DT", inplace=True)
# Write to CSV
table_content.to_csv("Output/NYT.csv", index=False)
    
print("==== Ending data fetch for NYT ====")
