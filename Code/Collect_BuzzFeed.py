...
print("==== Starting data fetch for BuzzFeed ====")

for index, row in table_sources.iterrows():

  if row['Source'] == 'BuzzFeed':
    feed_name = row['Feed']
    feed_link = row['Link']
  else:
    continue

  print("Fetching data for BuzzFeed: " + feed_name)
...
table_content = pd.read_csv("Output/BuzzFeed.csv")
# Backup data file
...
# Write to CSV
table_content.to_csv("Output/BuzzFeed.csv", index=False)

print("==== Ending data fetch for BuzzFeed ====")
